 ...
 */
function writeImgWithLink(blob: Blob, linkDownload = "img.png"): void {
 ...
 * @param blob Blob 对象
 * @param linkDownload 下载链接的文件名(注意包文件含拓展名)
 * 如果不支持或失败则回退到 link 方式
 */
export async function copyImg(blob: Blob, linkDownload = "img.png"): Promise<void> {
    try {
        await tryWriteImgWithModernAPI(blob)
        console.log("Blob has been copied using navigator.clipboard!")
    } catch (err) {
        console.log("Falling back to link: ", err)
        writeImgWithLink(blob, linkDownload)
    }
}
