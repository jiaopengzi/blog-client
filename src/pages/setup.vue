<!--
 * FilePath    : blog-client-nuxt\src\pages\setup.vue
 * Author      : jiaopengzi
 * Blog        : https://jiaopengzi.com
 * Copyright   : Copyright (c) 2026 by jiaopengzi, All Rights Reserved.
 * Description : 系统初始化 / 数据库配置页(原 spa views/setup 视图已并入本页面)
-->

<!--
 * 补充说明:
 * 纯 CSR(routeRules ssr:false); middleware "setup" 对齐 SPA setupMiddleware
 * (已初始化 → /not-found); 未初始化时展示 Pgsql/Redis/ES 配置表单, 提交成功后
 * 经 RestartDialog 倒计时跳转管理员注册页(SPA confirmFunc 语义)
-->
...
definePageMeta({ layout: false, name: "setup", middleware: "setup" })
...
<template>
    ...
    <div class="page">
        <!-- 纯 CSS 背景氛围层(网格+柔光), fixed 定位会盖住普通流, 内容容器已在样式中 position:relative -->
        <AccountBackdrop />
        <div class="setup">
...
// position:relative 保证内容叠于 fixed 背景氛围层之上
.setup {
    position: relative;
    display: flex;
