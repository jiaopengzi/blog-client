<!--
 * FilePath    : blog-client-nuxt\src\pages\login.vue
 * Author      : jiaopengzi
 * Blog        : https://jiaopengzi.com
 * Copyright   : Copyright (c) 2026 by jiaopengzi, All Rights Reserved.
 * Description : 登录页 (CSR, 原 views/login 视图已并入本页面)
-->

<!--
 * 补充说明:
 * definePageMeta/useHead 来自原 pages 包装, 逻辑与模板来自原视图
 * 保持原 CSR 语义: 整页模板位于 ClientOnly 内, 服务端不渲染登录表单;
 * 登录成功回跳依赖路由名 "login" (守卫/回跳引用)
-->

<script setup lang="ts">
// Nuxt 适配: useHead / useRouter 为 Nuxt 自动导入 (@unhead/vue、vue-router 非直接依赖)
...
import AccountBackdrop from "@/components/common/account-backdrop"
...
import type { LoginForm } from "@/components/views/login/types"

// 路由名与 SPA RouteNames.Login ("login") 对齐 (守卫/回跳依赖)
        ...
        if (valid) {
            ...
            if (userStore.getIsLogin) {
                ...
                if (redirectPath) {
                    ...
                    // 创建路由对象 (结构形状兼容 vue-router RouteLocationRaw)
                    const routeLocation = {
                        path: path,
                        query: queryParams,
                    }
...
<template>
    <ClientOnly>
        <div class="login-page">
            <!-- 纯 CSS 背景氛围层(网格+柔光), fixed 定位会盖住普通流, 卡片已在样式中 position:relative -->
            <AccountBackdrop />
            <!-- 滑动验证组件 -->
...
// 卡片入场动画; position:relative 保证卡片叠于 fixed 背景氛围层之上
.login-card {
    position: relative;
