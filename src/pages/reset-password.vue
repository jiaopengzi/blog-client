 ...
 * Description : 重置密码页(CSR, 原 views/reset-password 视图已并入本页面; definePageMeta/useHead 来自原 pages 包装, 逻辑与模板来自原视图)
-->

<!--
 * 补充说明:
 * 保持原 CSR 语义: 整页模板位于 ClientOnly 内, 服务端不渲染重置表单
-->
...
import type { ResetPasswordForm } from "@/components/views/reset-password/types"

definePageMeta({ layout: false, name: "reset-password" })
...
const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            const { data } = await resetPasswordAPI(req)
