...
export function useUtils(
    ...
    const updateRouterPush = async () => {
        ...
        if (queryParams.key_word) {
            await routerPushByParams(router, RouteNames.Search, queryParams)
            return
        }
    ...
    const updateQueryParams = async () => {
        const { hasPagination, hasQuery, result } = await parseRouteQuery(route.query, options as QueryParamsOptions<ViewPostRequest>)
        // 走搜索路由
        if (result.key_word) {
