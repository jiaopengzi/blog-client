<!--
 * FilePath    : blog-client-nuxt\src\components\views\user-public-profile\index.vue
 * Author      : jiaopengzi
 * Blog        : https://jiaopengzi.com
 * Copyright   : Copyright (c) 2026 by jiaopengzi, All Rights Reserved.
 * Description : 用户公开资料展示组件
-->
...
/**
 * 渲染 404 页面并保留当前访问路径, 避免地址栏丢失原始 URL.
 * bug02(260830-02): SPA 的 catch-all 为 /:pathMatch(.*)*, 按 name + pathMatch 的 replace 可落到
 * 404 视图; Nuxt 的 catch-all 是 pages/[...slug].vue(/:slug(.*)*), 且 /user/:username 优先级更高,
 * 同一写法会被解析成 "/"(跳回首页). 这里改用 showError 直接渲染 error.vue, 地址栏 URL 不变.
 */
const showNotFound = (): void => {
    showError(createError({ statusCode: 404, message: "用户不存在" }))
}
