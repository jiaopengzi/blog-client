/*
 * FilePath    : blog-client-nuxt\src\components\views\user-info\component\info\hooks\useInfo.ts
 * Author      : jiaopengzi
...
export interface UseInfoReturnType {
    ...
    formSize: Ref<"" | "small" | "default" | "large">
...
export function useInfo(): UseInfoReturnType {
    ...
    const formSize = ref<"" | "small" | "default" | "large">("default")

    const userStore = useUserStore()

    const { data: userData, avatar, isBindEmail } = storeToRefs(userStore)

    const userNameDisabled = ref(true) // 用户名不可用, 默认 true(不能编辑)
    ...
    /**
     * @description: 未绑定邮箱时的统一拦截: 弹出绑定邮箱弹窗并警告提示 (调整 260829-03)
     * @return void
     */
    const promptBindEmail = () => {
    ...
    const submitForm = async (formEl: FormInstance | undefined) => {
        ...
        // 调整 260829-03 (1): 未绑定邮箱时禁止保存个人信息, 与路由守卫/分区拦截同语义
