<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\app-option\index.vue
 * Author      : jiaopengzi
...
// payload 瘦身适配 (bugfix 260825-03): optionsStore.app_options 已改为公开端白名单瘦身版,
// 后台表单需要全量配置, 改为挂载时经 getAPPOptionAPI 自取全量, 不再依赖 store 中的全量数据
const fullOptionData = reactive<GetAPPOptionResponse>({} as GetAPPOptionResponse)
...
const submitForm = async () => {
    ...
    for (const key in optionDataTar) {
        if (Object.prototype.hasOwnProperty.call(optionDataTar, key)) {
            ...
            const itemSrc = fullOptionData[key as keyof GetAPPOptionResponse]

            // 判断是否更新
            const valSrc = fullOptionData[key as keyof GetAPPOptionResponse].value
...
onBeforeMount(async () => {
    // payload 瘦身适配 (bugfix 260825-03): 自取全量配置填充表单, 不依赖 store 中的瘦身版
    ...
    Object.assign(fullOptionData, data)
