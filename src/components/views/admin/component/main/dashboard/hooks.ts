/*
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\dashboard\hooks.ts
 * Author      : jiaopengzi
 * Blog        : https://jiaopengzi.com
 * Copyright   : Copyright (c) 2025 by jiaopengzi, All Rights Reserved.
 * Description : 管理台面板组合式函数
 */
...
export function useDashboard() {
    // 安全收敛(2026-08-28 决策): 客户端 git tag 不再进 bundle(app.config 注入已回退),
    // 管理台版本对比经同源 /VERSION 运行时拉取(与外部探活共用同一唯一暴露通道)
    const versionClient = ref("dev")
    ...
    const applyRemoteVersionUpdateState = (overview: RemoteVersionOverviewRes | null = remoteVersionOverview.value) => {
        const versionUpdateState = buildDashboardVersionUpdateState(overview, versionServer.value.version, versionClient.value)
