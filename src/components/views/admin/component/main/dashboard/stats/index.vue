<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\dashboard\stats\index.vue
 * Author      : jiaopengzi
...
const handleCardClick = async (name: string) => {
    if (name === StatsResKey.CommentCountPending) {
        // bug01(260829-05): Nuxt 的 admin 子页为 [...slug] catch-all, SPA 路由名 (comment) 不在
        // Nuxt 路由表中, 按 name push 会抛 "No match", 对齐 useAdd 的路径跳转
        await router.push({
            path: `/admin/${RouteNames.Comment}`,
            query: {
