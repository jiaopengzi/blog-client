...
interface FormValidationOptions {
    form: {
        ...
        push_time?: Ref<PgSqlDateTime | undefined> // 推送时间
        push_status?: Ref<NotificationPushStatus | undefined> // 是否已推送, 1 未推送, 2 推送中, 3 已推送
        category?: Ref<NotificationCategory> // 通知类别
        status?: Ref<NotificationStatus> // 状态, 1 禁用, 2 启用
        format?: Ref<NotificationFormat> // 通知格式, 1 html, 2 纯文本
    }
...
export function useFormValidation(options: FormValidationOptions): {
    ...
    function checkToListValidator(rule: unknown, value: string, callback: (error?: string | Error | undefined) => void): void {
        ...
        // 遍历 toListArray 检查每个接收人, 是否符合邮箱或者角色
