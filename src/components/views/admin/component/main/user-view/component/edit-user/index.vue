<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\user-view\component\edit-user\index.vue
 * Author      : jiaopengzi
...
const emit = defineEmits<{
    (event: "edit-user-status", value: boolean): void
}>()

const props = defineProps<{
    editUserData: EditUserByAdminForm // 待编辑的用户数据
    roles: Role[]
}>()
...
// 监听 props.editUserData 变化, 更新页面数据
watch(
    () => props.editUserData,
    (newVal) => {
        updateEditUserForm(newVal)
        getUserInfo()
    },
    { deep: true },
)
