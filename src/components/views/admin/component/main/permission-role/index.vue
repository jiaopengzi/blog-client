<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\permission-role\index.vue
 * Author      : jiaopengzi
...
import { type Permission, usePermissionRoleStore } from "@/stores/permissionRole"
...
import type { PermissionRow, Role } from "@/components/views/admin/component/main/permission-role"
...
const permissionRoleForm = reactive<PermissionRole>({
    permission_name: PermissionNames.AddMedia,
    role_name: "",
    limit_count: 0, // 限制次数, 0 表示不限制, >0 表示次数
    limit_period: 0, // 限制时间段, 0 表示不限制, >0 表示秒数
})
...
/**
 * @description: 大于等于 0 的整数 Validator (el-form-item 需要绑定对应的prop才能触发校验)
 * @param rule 校验规则
 * @param value 对应输入框的值
 * @param callback 回调函数
 */
...
const submitUpsertForm = debounce(100, async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            // 当 limit_count 为 0 且 limit_period 不等于 0 时, 警告提示
