<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\comment\index.vue
 * Author      : jiaopengzi
...
<template>
    <section>
        ...
        >
            <template #category>
                <!-- v-for 循环 commentCountGroup 生成按钮 -->
...
// 监控 commentOperationSelect 更改
watch(commentOperationSelect, async (newVal) => {
    if (newVal) {
        // 将 commentStatusOperationList 状态更改为 newVal
...
const editRow = (index: number, row: TableData) => {
    ...
    if ("post_info" in row) {
        ...
        mentions.value.push({
            ...
            apply: `[@${row.user_info.user_display_name}](${window.location.origin}/user/${row.user_info.user_name}) `,
        })
