<!--
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\coupon\component\view\index.vue
 * Author      : jiaopengzi
...
watch(
    ...
    (newType) => {
        if (newType === CouponDiscountType.FixedAmount) {
            // 固定金额折扣
            amountUnit.value = "元"
            amountMax.value = 999999 // 最大值为 999999 元
            amountPrecision.value = 2 // 精度为 2
            amountStep.value = 0.1 // 步长为 0.1 元
        } else if (newType === CouponDiscountType.Percentage) {
            // 百分比折扣
            amountUnit.value = "%"
            amountMax.value = 100 // 最大值为 100%
            amountPrecision.value = 0 // 精度为 0
            amountStep.value = 1 // 步长为 1%
        }
    },
    {
