/*
 * FilePath    : blog-client-nuxt\src\components\views\admin\component\main\coupon\component\view\hooks.ts
 * Author      : jiaopengzi
...
interface FormValidationOptions {
    form: {
        ...
        status: Ref<CouponStatus> // 状态, 1 禁用, 2 启用
    }
}
...
export function useFormValidation(options: FormValidationOptions): {
    ...
    function checkCodeExcludingIDValidator(rule: unknown, value: string, callback: (error?: string | Error | undefined) => void): void {
        ...
        const req: CheckCouponCodeExcludingIDRequest = {
            excluding_id: form.id?.value || "", // 需要排除的 ID
            code: value,
        }
     ...
     * @return FormRules<ViewForm> 表单校验规则, trigger: 'blur' 表示失去焦点时校验, 'change' 表示值改变时校验
     */
