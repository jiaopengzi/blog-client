 ...
 * @LastEditTime : 2024-10-18 19:19:14
...
<template>
    ...
    >
        <VideoWatermark
            :text-watermark="localPlayerProps.textWatermark"
            :logo-watermark="localPlayerProps.logoWatermark"
        >
            <!-- video元素不使用默认的  controls-->
            <video
                ref="videoRef"
                :src="localPlayerProps.src"
                :poster="localPlayerProps.poster"
...
import { ref, computed, watch, watchEffect, onBeforeUnmount, onMounted, useTemplateRef } from "vue"
...
import type { LanguageKey, PlayLevelLabel, PlayerProps } from "@/components/player"
...
const updatePlayerByControls = (playerProps: PlayerProps) => {
    localPlayerState.value.updateState(playerProps)
}

// 根据当前环境更新 isMobile
localPlayerState.value.setIsMobile(
...
// 判断是否是 Iphone
localPlayerState.value.setIsIphone(/iPhone/i.test(navigator.userAgent))
...
// 状态
const showPlayButton = computed(() => localPlayerProps.value.playStatus !== PlayStatus.PLAYING)
const showLoader = computed(() => localPlayerProps.value.playStatus === PlayStatus.BUFFERING)

// 切换播放暂停
const togglePlayPause = () => {
    console.log("togglePlayPause")
    console.log("前", localPlayerState.value.isPlaying)
    console.log("前1", localPlayerProps.value.playStatus)
    localPlayerState.value.togglePlayPause()
    console.log("后", localPlayerState.value.isPlaying)
    console.log("后1", localPlayerProps.value.playStatus)
}

// 处理视频页面双击事件
const handleDblclick = () => {
    // 判断是否是网页全屏
    if (localPlayerProps.value.isWebFullScreen) {
        localPlayerState.value.toggleWebFullScreen()
        return
    }

    // 判断是否是画中画
    if (localPlayerProps.value.isPictureInPicture) {
        localPlayerState.value.togglePictureInPicture()
        return
    }

    // 判断是否是全屏
    localPlayerState.value.toggleFullScreen()
}
...
// 视频播放结束
const handleEnded = () => {
    localPlayerState.value.end()
}

// 视频缓冲事件
const handleWaiting = () => {
    // 如果是播放状态, 则缓冲,否则保持原状态
    if (localPlayerProps.value.playStatus === PlayStatus.PLAYING) localPlayerState.value.buffering()
}

// 处理可以播放事件
const handleCanplay = () => {
    // 如果是缓冲状态, 则播放,否则保持原状态
    if (localPlayerProps.value.playStatus === PlayStatus.BUFFERING) localPlayerState.value.play()
}
...
const handleProgressBuffered = () => {
    if (videoRef.value) {
        ...
        if (buffered.length > 0) {
            ...
            localPlayerState.value.setBuffered(bufferedEnd)
        }
    }
}
...
const isShowSubtitles = computed(() => {
    if (
        localPlayerProps.value.subtitles &&
        localPlayerProps.value.subtitles.selectedSubtitlesLanguage
    ) {
        ...
        return !Object.keys(DisabledSubtitles).includes(
            localPlayerProps.value.subtitles.selectedSubtitlesLanguage as LanguageKey,
        )
    }
    return false
})
...
const subtitlesSrc = computed(() => {
    if (
        localPlayerProps.value.subtitles &&
        localPlayerProps.value.subtitles.availableSubtitles &&
        localPlayerProps.value.subtitles.selectedSubtitlesLanguage
    ) {
...
const srclang = computed(() => {
    if (
        ...
        return localPlayerProps.value.subtitles.selectedSubtitlesLanguage
    }
    return ""
})
...
watchEffect(() => {
    if (videoContainerRef.value) {
        videoContainerRef.value.style.width = localPlayerProps.value.size.width + "px"
        videoContainerRef.value.style.height = localPlayerProps.value.size.height + "px"
    }
})
...
const adjustSizeExitFullscreen = () => {
    if (videoContainerRef.value && videoRef.value) {
        videoRef.value.style.width = localPlayerProps.value.size.width + "px"
        videoRef.value.style.height = localPlayerProps.value.size.height + "px"
        videoContainerRef.value.style.width = localPlayerProps.value.size.width + "px"
        videoContainerRef.value.style.height = localPlayerProps.value.size.height + "px"
    }
}
...
const handleFullscreenChange = () => {
    if (screenfull.isEnabled) {
        // 更新 store 中的全屏状态
        localPlayerState.value.setIsFullScreen(screenfull.isFullscreen)
...
watchEffect(() => {
    ...
    if (localPlayerProps.value.isFullScreen) {
        if (videoContainerRef.value && videoRef.value) {
            if (screenfull.isEnabled) {
                adjustSizeFullscreen()
            ...
            if (localPlayerProps.value.isIphone) {
                ...
                if (videoElement.webkitEnterFullscreen) {
                    ...
                    localPlayerState.value.setIsFullScreen(false) // 只要进入全屏，就设置为 false,退出后也是false
                }
            }

            // 移动端时 锁定屏幕方向为横屏
            if (localPlayerProps.value.isMobile && typeof orientation.lock === "function") {
...
// 监控画中画状态
watchEffect(() => {
    if (videoRef.value && localPlayerProps.value.isPictureInPicture) {
        videoRef.value.requestPictureInPicture()
    }
    if (!localPlayerProps.value.isPictureInPicture && document.pictureInPictureElement) {
...
watchEffect(() => {
    if (videoRef.value) {
        console.log("playStatus=============>", localPlayerProps.value.playStatus)
        localPlayerProps.value.playStatus === PlayStatus.PLAYING
            ? videoRef.value.play()
...
//     (newVal) => {
//         if (videoRef.value) {
//             console.log("playStatus=============>", newVal)
//             newVal.playStatus === PlayStatus.PLAYING
//                 ? videoRef.value.play()
//                 : videoRef.value.pause()
//         }
//     },
...
const handleTimeupdate = () => {
    if (localPlayerProps.value.isUserInput || localPlayerProps.value.playProgress.isDragging) return
    if (videoRef.value) {
        localPlayerState.value.setUserInput(false)
        localPlayerState.value.setCurrentTime(videoRef.value.currentTime)
    }
}

// 监听用户输入进度变化
watchEffect(() => {
    if (!localPlayerProps.value.isUserInput) return
    if (videoRef.value) {
        videoRef.value.currentTime = localPlayerProps.value.playProgress.currentTime

        // 更新缓存进度
        if (!localPlayerProps.value.playProgress.isDragging) {
            handleProgressBuffered()
        }
        localPlayerState.value.setUserInput(false)
    }
...
// 监控音量变化
watchEffect(() => {
    if (videoRef.value) videoRef.value.volume = localPlayerProps.value.volume.volume / 100
})

// 监控播放速度变化
watchEffect(() => {
    if (videoRef.value) videoRef.value.playbackRate = localPlayerProps.value.playbackRate
})

// 监控是否循环播放
watchEffect(() => {
    if (videoRef.value) videoRef.value.loop = localPlayerProps.value.isLoop
})
...
const updateStore = () => {
    if (videoRef.value) {
        localPlayerState.value.setCurrentTime(videoRef.value.currentTime)
        localPlayerState.value.setDuration(videoRef.value.duration)
        localPlayerState.value.setIsDragging(false)
        localPlayerState.value.setPlaybackRate(videoRef.value.playbackRate)
        videoRef.value.loop = localPlayerProps.value.isLoop
        videoRef.value.volume = localPlayerProps.value.volume.volume / 100
    }
}
...
const loadHls = () => {
    if (Hls.isSupported()) {
        ...
        // 加载视频源
        hls.loadSource(localPlayerProps.value.src)
        ...
        hls.on(Hls.Events.MANIFEST_PARSED, () => {
            ...
            // 更新 store 中的清晰度信息
            localPlayerState.value.setPlayLevelAllLevels(localLevels)
        })
        ...
        hls.on(Hls.Events.LEVEL_SWITCHED, function (event, data) {
            ...
            localPlayerState.value.setSelectedPlayLevel(selectedLevel as PlayLevelLabel)
            // TODO 切换清晰度时, 显示提示信息
        })
        ...
        watchEffect(() => {
            ...
            if (levels) {
                const levelIndex = levels.findIndex(
                    (level) =>
                        getVideoQualityLabel(level.height) ===
                        localPlayerProps.value.playLevel.level,
                )
                if (levelIndex !== -1 && hls) {
...
watchEffect(() => {
    if (localPlayerProps.value.isIphone) {
        localPlayerState.value.setPoster("")
    }
})
...
const updateVideo = () => {
    // 判断视频类型
    if (localPlayerProps.value.mediaType === MediaTypes.HLS) {
        // 先销毁 hls 实例再加载
        // if (hls) {
    ...
    if (localPlayerProps.value.mediaType in [MediaTypes.MP4, MediaTypes.WEBM]) {
...
watchEffect(() => {
    ...
    if (localPlayerProps.value.src && videoRef.value) {
        localPlayerState.value.setSubtitlesByVideoHashIdAuto() // 自动更新字幕
        updateVideo()
    }
})
...
onBeforeUnmount(() => {
    ...
    // 销毁 hls 实例 destroy
    localPlayerState.value.destroy()
})
</script>
