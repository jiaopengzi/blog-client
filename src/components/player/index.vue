...
const subtitlesSrc = computed(() => {
  if (
    ...
    subtitles.value.selectedSubtitlesLanguage
  ) {
    ...
    const selectedSubtitle =
      availableSubtitles[subtitles.value.selectedSubtitlesLanguage as keyof typeof Language]
...
watchEffect(() => {
  if (videoRef.value)
    playStatus.value === PlayStatus.PLAYING ? videoRef.value.play() : videoRef.value.pause()
})
    ...
    watchEffect(() => {
      ...
      if (levels) {
        const levelIndex = levels.findIndex(
          (level) => getVideoQualityLabel(level.height) === playLevel.value.level
        )
        if (levelIndex !== -1 && hls) {
...
const updateVideo = () => {
  ...
  if (mediaType.value in [MediaTypes.MP4, MediaTypes.WEBM]) {
