...
import { CommentStatusCode, PostType, type UpdatePostRequest } from "@/api/post/common"
...
export function useEdit(
    ...
    // 初始化数据
    const getDataOnBeforeMount = async (): Promise<boolean> => {
        if (!postInfoForm.id) {
            return false
        }
        ...
        const resolution = resolvePostEditLoadError(res.data.code, res.data.msg, postType)
        userStore.setIsEditing(false)

        if (resolution.showNoPermission) {
            return true
        }

        if (resolution.redirectRouteName) {
