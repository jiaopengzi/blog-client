 ...
 * @LastEditTime : 2023-12-20 15:45:55
 * @FilePath     : \blog-client\src\components\common\editor\codemirror\Codemirror.vue
...
const registerHotKeys = () => {
    Object.entries(MardkdownEditorCommands).forEach((item) => {
        ...
        if (hotKey) {
            watch(keys[hotKey], (v) => {
                if (v)
                    editorInsertFormatContent(cmView, MardkdownEditorCommands[item[0]])
            })
        }
    })
}
...
// 执行按钮命令
const runCommand = (commandName: MardkdownEditorCommandsKeyType): void => {
    if (commandName) {
