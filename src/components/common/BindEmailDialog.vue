 ...
 * @LastEditTime : 2023-10-22 11:38:56
...
import type { BindEmailRequest, BindEmailResponse } from '@/api/user/BindEmail'
...
const submitForm = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            const requestData: string = JSON.stringify(req)// 将请求对象 req 转换为字符串
            const res: AxiosResponse<BindEmailResponse> = await bindEmailByJosn(requestData)// 发送请求，并返回Promise
            const resStr: string = JSON.stringify(res)// 将 res 转换字符串
            const resObj: BindEmailResponse = JSON.parse(resStr).data// 将 resStr 转换为对象

            if (resObj.code === ResponseCode.UserBindEmailSuccess) {
                // 显示注册成功提示
                userStore.getUserInfoByToken(true) // 强制更新用户信息
                // userStore.changeShowDialogBindEmail(false)
