/*
 * FilePath    : blog-client-nuxt\src\stores\permissionRole.ts
 * Author      : jiaopengzi
...
// // feature01 (260829-08): PermissionNames 权限名枚举已抽至 @/api/permissionRole/permissionNames.ts
// // (后端接口契约的纯枚举, Nitro server 边界可直接引用); 此处 re-export 保持既有引用零改动
// export { PermissionNames } from "@/api/permissionRole/permissionNames"
...
export const usePermissionRoleStore = defineStore("permissionRole", {
    ...
    actions: {
        ...
        readPostDetailEditCache(scope: PostDetailEditCacheScope): boolean | null {
            const localStorageData = safeLocalStorage.getItem(LocalStorageKey.PostDetailEditEnable)
        ...
        writePostDetailEditCache(scope: PostDetailEditCacheScope, enabled: boolean): void {
            ...
            safeLocalStorage.setItem(LocalStorageKey.PostDetailEditEnable, JSON.stringify(payload))
        },
