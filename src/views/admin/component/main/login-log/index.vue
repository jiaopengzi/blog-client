...
<template>
    <BaseTable :pagination="pagination" :table-column="cols" :is-show-delete-all="true" :is-show-search="true"
        :search-str="search" :is-show-edit="false" @update-current-page="updateCurrentPage"
        @update-page-size="updatePageSize" @update-page-sizes="updatePageSizes" @delete-rows="deleteRows"
        ...
        <template #btns>
            ...
            <el-button type="danger" @click="handleDeleteN">
                删除 n 天前记录
            </el-button>
        </template>
    </BaseTable>
    ...
    handleConfirmCommon(async () => {
        ...
        await DeleteLoginLogByDayAPI(deleteLoginLogByDayRequest).then((res) => {
            if (res.data.code === ResponseCode.LoginLogDeleteByDaySuccess) {
                // 删除成功后重新获取用户列表
                getLoginLogsPaginate({ current_page: pagination.value.current_page, page_size: pagination.value.page_size, key_word: search.value })
...
onBeforeMount(async () => {
    console.log("13============")
    getValueFromQuery()
    await getLoginLogsPaginate({ current_page: pagination.value.current_page, page_size: pagination.value.page_size, key_word: search.value })
})
